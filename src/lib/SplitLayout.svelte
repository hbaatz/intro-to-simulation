<script>
    ...
    let {
        ...
        capturePointerPosition = false,
        // Scene state
        pointerDown = $bindable(),
        pointerPosition = $bindable(),
        referencePoints = null,
        additionalModels = [],
        additionalModelParameters = $bindable([]),
        onUpdateScene = null,
        onUpdateSimulation = null,
        // Util config
        readout = null
    } = $props();
        ...
        <div class="simControls">
            <div style="display: flex;">
                <div class="buttonGroup">
                    <Simulation {stepFunc} {initialState} {constants} {onUpdateSimulation} bind:currentState bind:positionHistory bind:currentTime {stopCondition} active={!errorMsg}/>
                </div>
                {#if readout}
                    <div class="spacer"></div> 
                    <div class="buttonGroup">
                        <Readout value={readout} active={!errorMsg}/>
                    </div>
                {/if}
                {#if referenceCode}            
                    <div class="spacer"></div>           
            ...
            <div class="spacer"></div>
            <div class="buttonGroup">
                <Button icon={MousePointer} bind:value={showArrows}/>
                <Button icon={Spline} bind:value={showTrace}/>
...
<div class="sceneView">
    <Canvas>
        <Scene points={positionHistory} directions={currentState.slice(1).concat(constants.filter(v => Array.isArray(v) && v.length == 2))} 
            {referencePoints} {labels} {showTrace} {showArrows} {showTicks} {cameraFocus} {cameraZoom} {model} {additionalModels} {additionalModelParameters}
            {onUpdateScene}
            bind:pointerDown bind:pointerPosition
            />
    </Canvas>
</div>
